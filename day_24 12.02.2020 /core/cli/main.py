...
def cmd_task_list(args):
    """Handle the 'task-list' subcommand."""
    registry = setup_registry()
    kwargs = {}
    if args.status:
    ...
    result = registry.invoke("task.list", "assistant_agent", **kwargs)
    if not result.success:
        print(f"Error: {result.error}", file=sys.stderr)
        sys.exit(1)
    ...
    if not tasks:
        ...
        return
    print(f"{'ID':>4s}  {'Status':<12s} {'Priority':<10s} {'Effort':<8s} {'Title'}")
    print(f"{'─' * 4}  {'─' * 12} {'─' * 10} {'─' * 8} {'─' * 30}")
    for t in tasks:
        due = f" (due: {t['due_date']})" if t.get("due_date") else ""
        print(f"{t['id']:>4d}  {t['status']:<12s} {t['priority']:<10s} {t['effort']:<8s} {t['title']}{due}")


def cmd_task_status(args):
    """Handle the 'task-status' subcommand."""
    registry = setup_registry()
    result = registry.invoke("task.project_status", "assistant_agent")
    if not result.success:
        print(f"Error: {result.error}", file=sys.stderr)
        sys.exit(1)
    ps = result.data
    print(f"Total tasks: {ps.get('total', 0)}")
    ...
    if by_prio:
        parts = [f"{k}: {v}" for k, v in sorted(by_prio.items())]
        print(f"By priority (active): {', '.join(parts)}")
    blocked = ps.get("blocked", [])
    if blocked:
        print(f"\nBlocked ({len(blocked)}):")
        for b in blocked:
            print(f"  #{b['id']} {b['title']}: {b['blocked_by']}")
    overdue = ps.get("overdue", [])
    if overdue:
        print(f"\nOverdue ({len(overdue)}):")
        for o in overdue:
            print(f"  #{o['id']} {o['title']} (due: {o['due_date']})")
...
def main():
    ...
    # assistant
    asst_parser = subparsers.add_parser("assistant", help="Ask the unified project assistant")
    asst_parser.add_argument("question", type=str, help="The question or command to process")
    asst_parser.add_argument("--debug", action="store_true", help="Show debug info")
    asst_parser.set_defaults(func=cmd_assistant)

    # task-list
    tl_parser = subparsers.add_parser("task-list", help="List tasks with optional filters")
    tl_parser.add_argument("--status", type=str, default=None, help="Filter by status")
    tl_parser.add_argument("--priority", type=str, default=None, help="Filter by priority")
    tl_parser.add_argument("--tag", type=str, default=None, help="Filter by tag")
    tl_parser.add_argument("--limit", type=int, default=50, help="Max results")
    tl_parser.set_defaults(func=cmd_task_list)

    # task-status
    ts_parser = subparsers.add_parser("task-status", help="Show project-wide task status summary")
    ts_parser.set_defaults(func=cmd_task_status)
