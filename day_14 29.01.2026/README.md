...
https://monosnap.ai/file/YgK8uCdoaXq6zZAM63LrLGz1ZIILU6
...
- **Yandex Calendar**: интеграция с CalDAV, отдельный выбор даты/времени для календаря
- **Автоудаление из календаря**: при `/task_done` задача удаляется из Yandex Calendar
...
/task_add Купить продукты
  ...
  → Введите дату и время (today 14:30, tomorrow 9:00, 2026-02-01 10:00)
  → Готово!
```
...
├── yandex_calendar.py       # Интеграция с Яндекс Календарём (CalDAV)
