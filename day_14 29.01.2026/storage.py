    ...
    with get_connection() as conn:
        ...
        conn.execute("""
            CREATE TABLE IF NOT EXISTS user_token_settings (
                user_id INTEGER PRIMARY KEY,
                show_tokens INTEGER NOT NULL DEFAULT 1,
                updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            )
        """)
        conn.commit()
...
def get_user_show_tokens(user_id: int) -> bool:
    """
    Get whether token usage should be shown for a user.

    Args:
        user_id: Telegram user ID

    Returns:
        True if tokens should be shown (default), False otherwise
    """
    with get_connection() as conn:
        cursor = conn.execute(
            "SELECT show_tokens FROM user_token_settings WHERE user_id = ?",
            (user_id,)
        )
        row = cursor.fetchone()
        # Default to True (show tokens) if not set
        return row[0] == 1 if row else True


def set_user_show_tokens(user_id: int, show_tokens: bool) -> None:
    """
    Set whether token usage should be shown for a user.

    Args:
        user_id: Telegram user ID
        show_tokens: True to show tokens, False to hide
    """
    with get_connection() as conn:
        conn.execute("""
            INSERT INTO user_token_settings (user_id, show_tokens, updated_at)
            VALUES (?, ?, CURRENT_TIMESTAMP)
            ON CONFLICT(user_id) DO UPDATE SET
                show_tokens = excluded.show_tokens,
                updated_at = CURRENT_TIMESTAMP
        """, (user_id, 1 if show_tokens else 0))
        conn.commit()
    logger.info(f"User {user_id} token display set to {show_tokens}")
