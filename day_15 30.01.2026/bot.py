...
# Conversation states for /reminder email flow
(
    WAITING_FOR_EMAIL_DECISION,
...
EMAIL_CALLBACK_PREFIX = "email_notify:"
...
/reminder\\_email — Email уведомления
...
async def reminder_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    Usage:
        ...
        /reminder email    - Configure email notifications
    """
    user = update.effective_user
    args = context.args
    ...
    try:
        if subcommand == "on":
            ...
            if USE_HTTP_MCP:
                ...
                email_result = await mcp.get_email_preferences(user_id)
            else:
                result = await mcp.call_tool("reminder_get_preferences", {
                    "user_id": user_id
                })
                email_result = await mcp.call_tool("notification_get_email_preferences", {
                    "user_id": user_id
                })
            ...
            if result.success and result.data:
                ...
                # Get email status
                email_status = "выключены"
                email_addr = ""
                if email_result.success and email_result.data:
                    email_enabled = email_result.data.get("email_enabled", False)
                    email_recipient = email_result.data.get("email_recipient")
                    if email_enabled and email_recipient:
                        email_status = "включены"
                ...
                await update.message.reply_text(
                    ...
                    f"• Email уведомления: **{email_status}**{email_addr}",
                    parse_mode="Markdown"
                )
            else:
                await update.message.reply_text(
...
# =============================================================================
# Email Notification Flow Handlers
# =============================================================================

async def reminder_email_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """
    Start the email notification setup flow.

    Usage: /reminder_email
    """
    user = update.effective_user
    user_id = str(user.id)

    logger.info(f"User {user_id} starting email notification setup")

    # Check current email preferences
    if USE_HTTP_MCP:
        mcp = get_mcp_http_client()
        if mcp:
            result = await mcp.get_email_preferences(user_id)
            if result.success and result.data:
                current_email = result.data.get("email_recipient")
                email_enabled = result.data.get("email_enabled", False)
                if email_enabled and current_email:
                    context.user_data["current_email"] = current_email

    keyboard = [
        [
            InlineKeyboardButton("Yes", callback_data=f"{EMAIL_CALLBACK_PREFIX}yes"),
            InlineKeyboardButton("No", callback_data=f"{EMAIL_CALLBACK_PREFIX}no"),
        ]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    if current_email:
        message = (
            f"**Email Notification Setup**\n\n"
            f"Current email: `{current_email}`\n\n"
            "Would you like to update your email notification settings?"
        )
    else:
        message = (
            "**Email Notification Setup**\n\n"
            "Would you like to receive reminder notifications via email?\n\n"
            "When enabled, you'll receive an email copy of your task reminders."
        )
    ...
    return WAITING_FOR_EMAIL_DECISION


async def email_decision_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Handle the email notification decision (Yes/No)."""
    query = update.callback_query
    await query.answer()
    ...
    if not callback_data.startswith(EMAIL_CALLBACK_PREFIX):
        return WAITING_FOR_EMAIL_DECISION

    decision = callback_data[len(EMAIL_CALLBACK_PREFIX):]
    ...
    if decision == "no":
        ...
        if USE_HTTP_MCP:
            mcp = get_mcp_http_client()
            if mcp:
                await mcp.set_email_preferences(user_id, email_enabled=False)

        await query.edit_message_text(
            "Email notifications **disabled**.\n\n"
            "Use `/reminder_email` to enable them later.",
            parse_mode="Markdown"
        )
        return ConversationHandler.END

    # User wants to enable email - ask for address
    await query.edit_message_text(
        "Please enter your email address:\n\n"
        "_Example: user@example.com_\n\n"
        "Or /cancel to abort.",
        parse_mode="Markdown"
    )
...
async def email_address_received(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Save email preferences via MCP
    if USE_HTTP_MCP:
        mcp = get_mcp_http_client()
        if mcp:
            result = await mcp.set_email_preferences(user_id, email_enabled=True, email_recipient=email)
        ...
        client = get_task_tracker_client()
        if client:
            await client.call_tool("notification_set_email_preferences", {
                "user_id": user_id,
                "email_enabled": True,
    ...
    await update.message.reply_text(
        ...
        f"Email: `{email}`\n\n"
        "You will receive an email copy of your task reminders.\n"
        "Use `/reminder_email` to change settings.",
        parse_mode="Markdown"
    )

    logger.info(f"User {user_id} enabled email notifications: {email}")
    return ConversationHandler.END


async def email_setup_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Cancel the email setup flow."""
    await update.message.reply_text(
...
def main() -> None:
    ...
    # Email notification setup flow
    email_conv_handler = ConversationHandler(
        entry_points=[CommandHandler("reminder_email", reminder_email_command)],
        states={
            WAITING_FOR_EMAIL_DECISION: [
                CallbackQueryHandler(
                    email_decision_callback,
                    pattern=f"^{EMAIL_CALLBACK_PREFIX}"
                )
            ],
            WAITING_FOR_EMAIL_ADDRESS: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, email_address_received)
            ],
        },
        fallbacks=[CommandHandler("cancel", email_setup_cancel)],
        per_message=False,
    )
